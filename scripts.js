...
const PRODUCTION_KEY = 'stock_expert_production_v1'; // NOUVEAU: Productions
const RAW_MATERIALS_KEY = 'stock_expert_raw_materials_v1'; // NOUVEAU: Gestion Matière Première
const PRODUCT_NAMES_KEY = 'stock_expert_product_names_v1'; // NOUVEAU: Noms de produits mémorisés
const PRODUCTION_NAMES_KEY = 'stock_expert_production_names_v1'; // NOUVEAU: Noms de production mémorisés
...
// NOUVEAU: Variables globales pour mémoriser les noms de produits
let productNames = [];
let productionNames = [];
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        // Initialize Adapter (Cleans bad URLs)
        if (window.GoogleSheetsAdapter && window.GoogleSheetsAdapter.init) {
            window.GoogleSheetsAdapter.init();
        }
        ...
        if (window.GoogleSheetsAdapter && typeof window.GoogleSheetsAdapter.startRealtimeSync === 'function') {
            window.GoogleSheetsAdapter.startRealtimeSync().catch(err => {
                console.warn('Firebase realtime sync init failed', err);
            });
        }
        ...
        renderProductions(); // NOUVEAU: Charger l'historique des productions
        renderRawMaterials(); // NOUVEAU: Charger l'historique des matières premières
...
let productions = []; // NOUVEAU: Productions
let rawMaterials = []; // NOUVEAU: Gestion Matière Première
/*
...
async function loadDataFromDexieIfAvailable() {
    try {
        console.log('🔄 Loading from Dexie (Unified DB)...');
        // REMOVED 'localStorage.getItem(...) === null' checks to ensure Dexie is ALWAYS the source of truth if available
        ...
        if (typeof dbProductions !== 'undefined') {
            const dexieProductions = await dbProductions.getAll();
            if (dexieProductions && dexieProductions.length > 0) {
                productions = dexieProductions;
                console.log('✓ Productions loaded from Dexie (unified DB)');
            }
        }
        if (typeof dbRawMaterials !== 'undefined') {
            const dexieRawMaterials = await dbRawMaterials.getAll();
            if (dexieRawMaterials && dexieRawMaterials.length > 0) {
                rawMaterials = dexieRawMaterials;
                console.log('✓ Raw Materials loaded from Dexie (unified DB)');
            }
        }
    } catch (err) {
...
function loadData() {
    // GUARD: Only load from localStorage if the variable is empty (to avoid overwriting Dexie data)
    if (!inventory || inventory.length === 0 || (inventory.length === 1 && inventory[0].id === 1)) {
    ...
    if (!productions || productions.length === 0) {
        const storedProductions = localStorage.getItem(PRODUCTION_KEY);
        if (storedProductions) {
            try {
                productions = JSON.parse(storedProductions);
            } catch (err) {
                console.warn('Erreur parsing productions from localStorage', err);
                productions = [];
            }
        }
    }
    ...
    if (!rawMaterials || rawMaterials.length === 0) {
        const storedRawMaterials = localStorage.getItem(RAW_MATERIALS_KEY);
        if (storedRawMaterials) {
            try {
                rawMaterials = JSON.parse(storedRawMaterials);
            } catch (err) {
                console.warn('Erreur parsing rawMaterials from localStorage', err);
                rawMaterials = [];
            }
        }
    }

    // Always load names as they are usually small and non-critical
    const storedProductNames = localStorage.getItem(PRODUCT_NAMES_KEY);
    if (storedProductNames) {
        try {
            productNames = JSON.parse(storedProductNames);
        } catch (err) {
            console.warn('Erreur parsing productNames from localStorage', err);
            productNames = [];
        }
    }
    const storedProductionNames = localStorage.getItem(PRODUCTION_NAMES_KEY);
    if (storedProductionNames) {
        try {
            productionNames = JSON.parse(storedProductionNames);
        } catch (err) {
            console.warn('Erreur parsing productionNames from localStorage', err);
            productionNames = [];
        }
    }
...
const _debouncedStockSyncUp = (function () {
    let timer = null;
    return function () {
        if (window.__applyingRemoteState === true) return;
        if (typeof isReadOnlyUser === 'function' && isReadOnlyUser()) return;
        if (!navigator.onLine) return;

        try {
            const cfg = typeof getBackendConfig === 'function' ? getBackendConfig() : null;
            if (!cfg || (!cfg.enabled && !window.FIREBASE_CONFIG)) return;
        } catch (e) {
            if (!window.FIREBASE_CONFIG) return;
        }

        const statusEl = document.getElementById('backend-status');
        if (statusEl) statusEl.innerText = 'Envoi automatique vers Firebase...';

        window._isSyncPending = true;
        if (timer) clearTimeout(timer);
        timer = setTimeout(async () => {
            if (window.__applyingRemoteState === true) {
                window._isSyncPending = false;
                return;
            }
            try {
                if (window.GoogleSheetsAdapter && typeof window.GoogleSheetsAdapter.syncUp === 'function') {
                    await window.GoogleSheetsAdapter.syncUp();
                    if (statusEl) statusEl.innerText = 'Auto-sync terminé.';
                }
            } catch (err) {
                console.warn('[stock] autoSync failed:', err);
                if (statusEl) statusEl.innerText = 'Auto-sync échoué: ' + (err.message || err);
            } finally {
                window._isSyncPending = false;
            }
        }, 1000); // 1-second delay for premium, instant responsiveness!
    };
})();
window.addEventListener('online', _debouncedStockSyncUp);
...
function saveData() {
    ...
    localStorage.setItem(PRODUCTION_KEY, JSON.stringify(productions));
    localStorage.setItem(RAW_MATERIALS_KEY, JSON.stringify(rawMaterials));
    localStorage.setItem(PRODUCT_NAMES_KEY, JSON.stringify(productNames));
    localStorage.setItem(PRODUCTION_NAMES_KEY, JSON.stringify(productionNames));
    ...
    // Synchroniser avec Dexie (Base de données locale unifiée pour le Backup Cloud)
    if (typeof db !== 'undefined') {
        const tablesToSync = [
            { name: 'inventory', data: inventory },
            { name: 'dailyRecords', data: dailyRecords },
            { name: 'expenses', data: expenses },
            { name: 'income', data: income },
            { name: 'productions', data: productions },
            { name: 'rawMaterials', data: rawMaterials },
        ...
        tablesToSync.forEach(table => {
            if (db[table.name]) {
                // On ajoute la promesse au tableau pour pouvoir attendre la fin si nécessaire
                syncPromises.push(
                    db[table.name].clear()
                        .then(() => {
                            if (table.data && table.data.length > 0) {
                                return db[table.name].bulkPut(table.data);
                            }
                        })
                        .catch(err => console.warn(`Erreur sync Dexie table ${table.name}:`, err))
                );
            }
        });
    }
...
window.applyRemoteStateFromCloud = async function (state) {
    ...
    window.__applyingRemoteState = true;
    window._isApplyingRemoteData = true;
    try {
        if (Array.isArray(state.inventory)) inventory = state.inventory;
        if (Array.isArray(state.dailyRecords)) dailyRecords = state.dailyRecords;
        if (Array.isArray(state.expenses)) expenses = state.expenses;
        if (Array.isArray(state.income)) income = state.income;
        if (Array.isArray(state.productions)) productions = state.productions;
        if (Array.isArray(state.rawMaterials)) rawMaterials = state.rawMaterials;
        ...
        if (Array.isArray(state.productNames)) productNames = state.productNames;
        if (Array.isArray(state.productionNames)) productionNames = state.productionNames;
        if (Array.isArray(state.rhAppData) && state.rhAppData[0]) {
            localStorage.setItem('rh_app_data', JSON.stringify(state.rhAppData[0]));
        }

        if (state.companyInfo !== undefined) {
        ...
        if (state.themeColor !== undefined && state.themeColor !== null) {
            localStorage.setItem(THEME_COLOR_KEY, state.themeColor);
        }
        if (Array.isArray(state.userAccounts)) {
            if (typeof dbUserAccounts !== 'undefined' && dbUserAccounts.getAll && dbUserAccounts.clear && dbUserAccounts.bulkPut) {
                // Récupérer les hashedTokens locaux existants pour éviter qu'ils ne soient écrasés par les données brutes du Cloud
                const existingAccounts = await dbUserAccounts.getAll();
                const tokenMap = {};
                const checkMap = {};
                for (const acc of existingAccounts) {
                    if (acc.uid) {
                        if (acc.hashedToken) tokenMap[acc.uid] = acc.hashedToken;
                        if (acc.lastEmailVerificationCheck) checkMap[acc.uid] = acc.lastEmailVerificationCheck;
                    }
                }
                
                // Injecter les tokens existants dans les profils importés du Cloud
                for (const acc of state.userAccounts) {
                    if (acc.uid && tokenMap[acc.uid]) {
                        acc.hashedToken = tokenMap[acc.uid];
                    }
                    if (acc.uid && checkMap[acc.uid]) {
                        acc.lastEmailVerificationCheck = checkMap[acc.uid];
                    }
                }
                
                await dbUserAccounts.clear();
                if (state.userAccounts.length > 0) {
                    await dbUserAccounts.bulkPut(state.userAccounts);
                }
                console.log('[scripts] Loaded userAccounts from cloud restore (with token merge):', state.userAccounts.length);
            }
        }
...
function getBackendConfig() {
    try {
        ...
        if (!raw) {
            const firebaseConfig = window.FIREBASE_CONFIG ? JSON.stringify(window.FIREBASE_CONFIG) : '';
            return {
                enabled: !!firebaseConfig,
                baseUrl: firebaseConfig,
                token: '',
                autoPush: !!firebaseConfig
            };
        }
        const cfg = JSON.parse(raw);
        return {
            enabled: cfg.enabled !== undefined ? !!cfg.enabled : !!window.FIREBASE_CONFIG,
            baseUrl: cfg.baseUrl || (window.FIREBASE_CONFIG ? JSON.stringify(window.FIREBASE_CONFIG) : ''),
            token: cfg.token || '',
            autoPush: cfg.autoPush !== undefined ? !!cfg.autoPush : !!window.FIREBASE_CONFIG
        };
    } catch (err) {
...
async function pullDataFromBackend() {
    ...
    try {
        ...
        if (remote) {
            ...
            if (remote.income) localStorage.setItem(INCOME_KEY, JSON.stringify(remote.income));
            if (remote.productions) localStorage.setItem(PRODUCTION_KEY, JSON.stringify(remote.productions));
            if (remote.rawMaterials) localStorage.setItem(RAW_MATERIALS_KEY, JSON.stringify(remote.rawMaterials));
            ...
            // Synchronise avec Dexie
            if (remote.income) {
                if (typeof dbIncome !== 'undefined') await dbIncome.bulkPut(remote.income);
            }
            if (remote.productions) {
                if (typeof dbProductions !== 'undefined') await dbProductions.bulkPut(remote.productions);
            }
            if (remote.rawMaterials) {
                if (typeof dbRawMaterials !== 'undefined') await dbRawMaterials.bulkPut(remote.rawMaterials);
            }
...
async function tryLoadFromBackend() {
    ...
    try {
        ...
        if (remote) {
            ...
            if (remote.income) income = remote.income;
            if (remote.productions) productions = remote.productions;
            if (remote.rawMaterials) rawMaterials = remote.rawMaterials;
...
function switchTab(tab) {
    ...
    // determine whether tab was already active before we toggle anything
    const tabBtn = document.getElementById(`tab-${tab}`);
    const alreadyActive = tabBtn ? tabBtn.classList.contains('active') : false;
    ...
    // only reset dates when the tab actually changed; avoid overwriting user
    // input when switchTab is called redundantly (e.g. editExpense).  Keep the
    // same-day behaviour on real tab switches.
    if (!alreadyActive && typeof window.resetDates === 'function' && target) {
        window.resetDates(target);
    }

    if (tab === 'stats') updateCharts();
    if (tab === 'clients') switchSubTab('subtab-clients'); // Assurer l'affichage par défaut
    if (tab === 'production') {
        // Ensure raw material view is rendered when user opens the tab
        try {
            switchProductionTab('subtab-arrivage');
            renderRawMaterials();
        } catch (e) {
            console.warn('renderProductions error', e);
        }
    }
    if (tab === 'expenses') {
    ...
    if (tab === 'rapport') {
        try {
            // rapport-date will be handled by resetDates just below, but we still
            // want to run the helper that toggles its visibility and then render
            // the report.
            if (typeof toggleRapportDate === 'function') toggleRapportDate();
            renderDailyReport();
        } catch (e) {
            console.warn('renderDailyReport error', e);
        }
    }
...
function switchSubTab(subtab) {
    ...
    // determine if this subtab button was already active
    const subtabBtn = document.getElementById(`${subtab}-btn`);
    const alreadyActiveSub = subtabBtn ? subtabBtn.classList.contains('active') : false;

    if (targetContent) targetContent.classList.remove('hidden');
    if (subtabBtn) subtabBtn.classList.add('active');

    // after revealing the subtab make sure any date inputs are reset, but only
    // when the subtab actually changed. prevents a redundant click from
    // wiping a user-entered date when they trigger switchSubTab programmatically.
    // We intentionally do **not** reset dates when changing sub-tabs.  The
    // previous implementation ran resetDates on every switchSubTab call which
    // meant dates vanished if the user toggled between "production" and
    // "arrivage" or between "dépenses" and "rentrées".  Only main tab
    // switches should clear date fields, per the user requirement.
    // (The guard above handled repeated clicks but did not cover genuine
    // subtab transitions.)
    // If in the future certain sub-tabs need their own reset behaviour we can
    // add it selectively here.
...
// NOUVEAU: Gestion des onglets de Production
window.switchProductionTab = function (tab) {
    document.getElementById('subtab-production').classList.toggle('hidden', tab !== 'subtab-production');
    document.getElementById('subtab-arrivage').classList.toggle('hidden', tab !== 'subtab-arrivage');

    const productionBtn = document.getElementById('subtab-production-btn');
    const arrivageBtn = document.getElementById('subtab-arrivage-btn');

    if (tab === 'subtab-production') {
        productionBtn.classList.add('m3-btn-tonal');
        productionBtn.classList.remove('m3-btn-text');
        arrivageBtn.classList.add('m3-btn-text');
        arrivageBtn.classList.remove('m3-btn-tonal');
        renderProductions();
    } else {
        arrivageBtn.classList.add('m3-btn-tonal');
        arrivageBtn.classList.remove('m3-btn-text');
        productionBtn.classList.add('m3-btn-text');
        productionBtn.classList.remove('m3-btn-tonal');
    }
}
...
// Fonction utilitaire pour initialiser les dates
function initializeTodayDates(...elementIds) {
    const today = new Date().toISOString().split('T')[0];
...
window.initializeTodayDates = initializeTodayDates;
...
// NOUVEAU: Initialiser l'ajout d'un produit (Reset form)
window.initAddProduct = function () {
    ...
    document.getElementById('prodSalePrice').value = '';
    document.getElementById('prodSaleUnit').value = '';
    document.getElementById('prodType').value = 'finished';

    document.getElementById('productSubmitBtn').innerText = 'Ajouter Produit';

    if (typeof updateProductTypeUI === 'function') updateProductTypeUI();
...
window.handleProductSubmit = function (e) {
    ...
    const stock = parseInt(document.getElementById('prodStock').value) || 0; // Default to 0 if NaN
    ...
    if (isModification) {
        // Modification
        // Utiliser une comparaison souple pour l'ID au cas où
        const index = inventory.findIndex(p => String(p.id) === String(idInput));
        if (index !== -1) {
            // On préserve les anciens champs (ex: image, notes...)
            inventory[index] = {
                ...inventory[index],
                name,
                category,
...
async function toggleMode(mode) {
    ...
    document.getElementById('current-mode-title').innerText = isSale ? 'Point de Vente' : 'Gestion de Livraison';
    ...
    valText.innerText = isSale ? 'Valider & Facturer (F10)' : 'Sauvegarder Livraison (F10)';
    ...
    // Afficher/masquer les en-têtes de tableau selon le mode
    const headerSale = document.getElementById('header-sale');
    const headerDelivery = document.getElementById('header-delivery');
    const deliveryInfoContainer = document.getElementById('delivery-info-container');

    if (headerSale) headerSale.style.display = isSale ? 'table-row' : 'none';
    if (headerDelivery) headerDelivery.style.display = isSale ? 'none' : 'table-row';
    if (deliveryInfoContainer) deliveryInfoContainer.style.display = isSale ? 'none' : 'block';

    // Initialiser la date avec aujourd'hui
    if (!isSale && !document.getElementById('delivery-date').value) {
        const today = new Date().toISOString().split('T')[0];
...
function renderCart() {
    ...
    const isDelivery = currentMode === 'quote'; // Mode 'quote' is now delivery
    ...
    cart.forEach((item, idx) => {
        ...
        const deliveredQty = item.qty;
        const finalStock = initialStock - deliveredQty;
        ...
        // En mode livraison: Date, Produit, Stock Initial, Qté, Stock Final, Supprimer
        if (isDelivery) {
            const deliveryDate = document.getElementById('delivery-date').value || new Date().toISOString().split('T')[0];
            const formattedDate = new Date(deliveryDate).toLocaleDateString('fr-FR');
            tr.innerHTML = `
                        <td class="px-1 py-2 text-center font-mono text-gray-600 dark:text-gray-300">${formattedDate}</td>
                        <td class="px-1 py-2 font-medium truncate" title="${item.name}">${item.name}</td>
                        <td class="px-1 text-center font-mono text-gray-600 dark:text-gray-300">${initialStock}</td>
                        <td class="px-1"><input data-field="qty" type="number" min="0" max="9999" value="${item.qty}" class="table-input text-center font-bold text-brand border-brand-green/30 bg-green-50/30 dark:bg-green-900/20 rounded" onchange="updateCartItem(${idx}, 'qty', this.value)"></td>
                        <td class="px-1 text-center font-mono ${finalStock < 0 ? 'text-red-600 font-bold' : 'text-gray-600 dark:text-gray-300'}">${finalStock}</td>
                        <td class="px-1 text-center"><button onclick="remCart(${idx})" class="text-gray-400 hover:text-red-500 transition-colors"><i class="fas fa-times"></i></button></td>
                    `;
        } else {
            // Mode Vente: affichage complet
            const restCell = `<td class="text-center font-mono ${restColor}">${remaining}</td>`;
            tr.innerHTML = `
                        ...
                        <td class="px-1"><input data-field="qty" type="number" min="0" max="${prod ? prod.stock : 9999}" value="${item.qty}" ${prod && prod.stock <= 0 ? 'disabled' : ''} class="table-input text-center font-bold text-brand border-brand-green/30 bg-green-50/30 dark:bg-green-900/20 rounded" onchange="updateCartItem(${idx}, 'qty', this.value)"></td>
                        ${restCell}
    ...
    // Masquer la colonne Reste si mode Livraison
...
// --- GESTION DES LIVRAISONS (Mode 'quote' renommé) ---
...
function saveQuote() {
    ...
    // Renamed to saveDelivery for clarity
    return saveDelivery();
}
...
function saveDelivery() {
    ...
    // Récupérer les infos de livraison depuis les champs HTML
    const deliveryDateInput = document.getElementById('delivery-date').value;
    const clientNameInput = document.getElementById('delivery-client').value;

    const deliveryDate = deliveryDateInput ? new Date(deliveryDateInput + 'T00:00:00').toISOString() : new Date().toISOString();
    const clientName = clientNameInput || 'Client de Passage';

    // Créer un enregistrement de livraison avec stocks initial et final
    const deliveryItems = [];
    ...
    cart.forEach(item => {
        ...
        deliveryItems.push({
            ...
            total: deliveredQty * item.salePrice,
            totalCost: deliveredQty * item.purchasePrice,
            unit: item.saleUnit || 'Unit'
        });

        totalDeliveryValue += deliveredQty * item.salePrice;
    ...
    const deliveryRecord = {
        ...
        date: deliveryDate,
        items: deliveryItems,
        total: totalDeliveryValue,
        clientName: clientName,
        clientPhone: '',
    };

    quotes.unshift(deliveryRecord); // Keep using 'quotes' array for backward compatibility
    cart = [];
    saveData();

    // Afficher l'onglet Livraisons
    switchTab('clients');
    switchSubTab('subtab-quotes');

    if (window.showToast) window.showToast('Livraison enregistrée avec succès !', 'success');

    // Générer le bon de livraison PDF
    prepareInvoice(deliveryRecord.id, 'delivery');
}

window.loadQuoteToCart = function (id) {
    const quote = quotes.find(q => q.id === id);
    if (!quote) return alert("Livraison introuvable.");

    // Nettoyer le panier et charger les articles de la livraison
    cart = quote.items.map(item => {
        const prod = inventory.find(p => String(p.id) === String(item.productId) || p.name === item.name);
        return {
            id: prod ? prod.id : item.productId,
            name: item.name,
            category: prod ? prod.category : 'N/A',
            stock: prod ? prod.stock : item.finalStock,
            salePrice: item.price,
            saleUnit: item.unit || 'Unit',
    ...
    renderQuotes();
    switchTab('stock');
    alert("Livraison chargée dans le panier. Vous pouvez la modifier ou valider.");
}
...
window.addExpense = function (e) {
    ...
    initializeTodayDates('expense-date');
    saveData();
...
window.addProduction = function (e) {
    if (isReadOnlyUser()) {
        showToast("Accès en lecture seule : opération non autorisée.", "error");
        if (e && typeof e.preventDefault === 'function') e.preventDefault();
        return;
    ...
    const productName = document.getElementById('production-product-name').value;
    const rawQuantity = parseFloat(document.getElementById('production-raw-quantity').value);
    const finalQuantity = parseFloat(document.getElementById('production-final-quantity').value);
    const totalWeight = parseFloat(document.getElementById('production-total-weight').value) || 0;
    const description = document.getElementById('production-description').value;
    const productionId = document.getElementById('production-id').value;

    if (!date || !productName || isNaN(rawQuantity) || isNaN(finalQuantity)) {
        return showNotification('Veuillez remplir tous les champs obligatoires', 'error');
    }

    if (productionId) {
        const idx = productions.findIndex(p => p.id === parseInt(productionId));
        if (idx !== -1) {
            productions[idx] = {
                ...
                totalWeight,
                description,
                timestamp: productions[idx].timestamp
            };
        }
    } else {
        productions.unshift({
    ...
    // preserve the date input across successive entries
    const keepDateProd = date;
    document.getElementById('productionForm').reset();
    document.getElementById('production-id').value = '';
    const submitBtn = document.getElementById('production-submit-btn');
    if (submitBtn) submitBtn.innerHTML = '<i class="fas fa-plus"></i> Ajouter Production';
    if (keepDateProd) {
        document.getElementById('production-date').value = keepDateProd;
    } else {
        initializeTodayDates('production-date');
    }

    // NOUVEAU: Mémoriser le nom du produit
    if (productName && !productionNames.includes(productName)) {
        productionNames.push(productName);
    ...
    // WARNING: Year Check
    const prodYear = new Date(date).getFullYear();
    if (prodYear !== APP_YEAR) {
        showNotification(`Production enregistrée en ${prodYear}, visible uniquement en changeant l'exercice (Année actuelle: ${APP_YEAR}).`, 'warn');
    }
...
window.deleteProduction = function (id) {
    ...
    if (confirm('Supprimer cette production ?')) {
        ...
        saveData();
        renderProductions();
        showNotification('Production supprimée', 'success');
    }
}

function editProduction(id) {
    const prod = productions.find(p => p.id === id);
    if (!prod) return showNotification('Production introuvable', 'error');

    document.getElementById('production-id').value = prod.id;
    document.getElementById('production-date').value = new Date(prod.date).toISOString().slice(0, 10);
    document.getElementById('production-product-name').value = prod.productName;
    document.getElementById('production-raw-quantity').value = prod.rawQuantity;
    document.getElementById('production-final-quantity').value = prod.finalQuantity;
    document.getElementById('production-total-weight').value = prod.totalWeight;
    document.getElementById('production-description').value = prod.description || '';

    const submitBtn = document.getElementById('production-submit-btn');
    if (submitBtn) submitBtn.innerHTML = '<i class="fas fa-save"></i> Sauvegarder';

    setTimeout(() => document.getElementById('production-product-name').focus(), 120);
}

window.renderProductions = function () {
    const filterVal = document.getElementById('productionFilter')?.value || 'all';
    let productionsList = filterByTime(productions, filterVal).sort((a, b) => new Date(b.date) - new Date(a.date));

    const tbody = document.getElementById('productionTableBody');
    if (!tbody) return;

    if (productionsList.length === 0) {
        tbody.innerHTML = '<tr class="hover:bg-gray-50 dark:hover:bg-gray-700"><td colspan="8" class="px-2 py-2 text-center text-gray-500">Aucune production enregistrée</td></tr>';
        return;
    }

    tbody.innerHTML = productionsList.map(prod => {
        const prodDate = new Date(prod.date);
        const dateStr = prodDate.toLocaleDateString('fr-FR');
        const timeStr = prodDate.toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' });

        return `
            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer">
                <td class="px-2 py-2" data-label="Date">${dateStr}</td>
                <td class="px-2 py-2" data-label="Heure">${timeStr}</td>
                <td class="px-2 py-2 font-medium" data-label="Produit">${prod.productName}</td>
                <td class="px-2 py-2 text-right" data-label="Matière (kg)">${prod.rawQuantity.toFixed(2)}</td>
                <td class="px-2 py-2 text-right" data-label="Produit (kg)">${prod.finalQuantity.toFixed(2)}</td>
                <td class="px-2 py-2 text-right" data-label="Poids Total">${prod.totalWeight.toFixed(2)}</td>
                <td class="px-2 py-2 truncate" data-label="Détail">${prod.description || '-'}</td>
                <td class="px-2 py-2 text-center" data-label="Actions">
                    <button onclick="editProduction(${prod.id})" class="text-blue-500 hover:text-blue-700 mr-2"><i class="fas fa-edit"></i></button>
                    <button onclick="deleteProduction(${prod.id})" class="text-red-400 hover:text-red-600"><i class="fas fa-trash-alt"></i></button>
                </td>
            </tr>
        `;
    }).join('');
...
window.exportProductionPDF = function () {
    ...
    if (productionsList.length === 0) {
        return showNotification('Aucune production à exporter', 'warn');
    }

    // Calculer les totaux par produit
    const productTotals = {};
    productionsList.forEach(prod => {
        const normalizedName = prod.productName ? prod.productName.trim() : '';
        ...
        productTotals[normalizedName].rawQty += prod.rawQuantity;
        productTotals[normalizedName].finalQty += prod.finalQuantity;
        productTotals[normalizedName].totalWeight += prod.totalWeight;
    });

    // Préparer les données du tableau
    const tableData = productionsList.map(prod => {
        const prodDate = new Date(prod.date);
        const dateStr = prodDate.toLocaleDateString('fr-FR');
        const timeStr = prodDate.toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' });
        return [
            ...
            timeStr,
            prod.productName,
            prod.rawQuantity.toFixed(2),
            prod.finalQuantity.toFixed(2),
            prod.totalWeight.toFixed(2),
            prod.description || '-'
        ];
    });

    // Ajouter une ligne de total pour chaque produit unique
    const uniqueProducts = Object.keys(productTotals).sort();
    const totalRows = uniqueProducts.map(prodName => {
        const totals = productTotals[prodName];
        return [
            '---',
            'TOTAL',
            prodName,
            totals.rawQty.toFixed(2),
            totals.finalQty.toFixed(2),
    ...
    if (logoData) {
        try {
            doc.addImage(logoData, 'JPEG', 15, yPosition, 25, 25);
        } catch (e) {
            console.warn("Logo error", e);
        }
    }
    ...
    doc.setFontSize(18);
    doc.setTextColor(31, 122, 62);
    doc.text("RAPPORT DE PRODUCTION", pageWidth / 2, yPosition + 8, {
        align: 'center'
    });

    doc.setFontSize(10);
    doc.setTextColor(100);
    doc.text(`Date d'export: ${new Date().toLocaleDateString('fr-FR')}`, pageWidth / 2, yPosition + 14, {
        align: 'center'
    });
    ...
    doc.autoTable({
        ...
        didDrawCell: (data) => {
            // Mettre en gras et colorer les lignes de total
            if (data.row.index >= tableData.length && data.row.index < tableData.length + totalRows.length) {
                data.cell.styles.fillColor = [220, 240, 220]; // Léger vert
                data.cell.styles.fontStyle = 'bold';
                data.cell.styles.textColor = [31, 122, 62];
            }
        }
    ...
    // RéSUMé
    const totalRawQty = Object.values(productTotals).reduce((sum, t) => sum + t.rawQty, 0);
    const totalFinalQty = Object.values(productTotals).reduce((sum, t) => sum + t.finalQty, 0);
    const totalProductions = productionsList.length;
    ...
    doc.text("RéSUMé DE PRODUCTION", 20, finalY + 7);
    doc.setFontSize(9);
    doc.setTextColor(0);
    doc.setFont(undefined, 'normal');
    doc.text(`Total Productions: ${totalProductions} | Matière Utilisée: ${totalRawQty.toFixed(2)} kg | Produit Final: ${totalFinalQty.toFixed(2)} kg`, 20, finalY + 13);
    ...
    const hfSuffix = getFilterFileSuffix(filterVal);
    doc.save(`Production_${hfSuffix}_${new Date().toISOString().slice(0, 10)}.pdf`);
    showNotification('PDF exporté avec succès', 'success');
}

window.exportProductionXLSX = function () {
    const filterVal = document.getElementById('productionFilter')?.value || 'all';
    let productionsList = filterByTime(productions, filterVal).sort((a, b) => new Date(b.date) - new Date(a.date));
    ...
    const sheetData = productionsList.map(prod => {
        const prodDate = new Date(prod.date);
        const dateStr = prodDate.toLocaleDateString('fr-FR');
        const timeStr = prodDate.toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' });
        return {
            ...
            'Matière (kg)': prod.rawQuantity,
            'Produit (kg)': prod.finalQuantity,
            'Poids Total': prod.totalWeight,
            'Détail': prod.description || ''
        };
    });
...
// --- GESTION MATIÈRE PREMIÈRE ---

// NOUVEAU: Auto-complétion pour les noms de produits
window.setupProductNameAutocomplete = function (inputId, listId, namesArray) {
    ...
    inputEl.addEventListener('input', function () {
        ...
        matches.slice(0, 10).forEach(name => {
            ...
            option.className = 'p-2 cursor-pointer hover:bg-blue-100 dark:hover:bg-blue-900 border-b';
...
function computeRawMaterialStock() {
    const arrivedQty = parseFloat(document.getElementById('raw-material-arrived-qty').value) || 0;
    const outQty = parseFloat(document.getElementById('raw-material-out-qty').value) || 0;
    const finalStock = Math.max(0, arrivedQty - outQty);
    document.getElementById('raw-material-final-stock').value = finalStock.toFixed(2);
}

window.addRawMaterial = function (e) {
    if (isReadOnlyUser()) {
        showToast("Accès en lecture seule : opération non autorisée.", "error");
        if (e && typeof e.preventDefault === 'function') e.preventDefault();
        return;
    ...
    e.preventDefault();
    const date = document.getElementById('raw-material-date').value;
    const productName = document.getElementById('raw-material-product-name').value;
    const arrivedQty = parseFloat(document.getElementById('raw-material-arrived-qty').value);
    const outQty = parseFloat(document.getElementById('raw-material-out-qty').value) || 0;
    const finalStock = parseFloat(document.getElementById('raw-material-final-stock').value) || 0;
    const rawQuantity = parseFloat(document.getElementById('raw-material-raw-quantity').value) || 0;
    const finalQuantity = parseFloat(document.getElementById('raw-material-final-quantity').value) || 0;
    const totalWeight = parseFloat(document.getElementById('raw-material-total-weight').value) || 0;
    const description = document.getElementById('raw-material-description').value;
    const rawMaterialId = document.getElementById('raw-material-id').value;

    if (!date || !productName || isNaN(arrivedQty)) {
        return showNotification('Veuillez remplir tous les champs obligatoires', 'error');
    }

    if (rawMaterialId) {
        const idx = rawMaterials.findIndex(rm => rm.id === parseInt(rawMaterialId));
        if (idx !== -1) {
            rawMaterials[idx] = {
                id: parseInt(rawMaterialId),
                date: new Date(date).toISOString(),
                productName,
                arrivedQty,
                outQty,
                ...
                totalWeight,
                description,
                timestamp: rawMaterials[idx].timestamp
            };
        }
    } else {
        rawMaterials.unshift({
            id: Date.now(),
            date: new Date(date).toISOString(),
            productName,
            arrivedQty,
    ...
    document.getElementById('raw-material-final-stock').value = '';
    const submitBtn = document.getElementById('raw-material-submit-btn');
    if (submitBtn) submitBtn.innerHTML = '<i class="fas fa-plus"></i> Ajouter Matière';
    initializeTodayDates('raw-material-date');
    ...
    // WARNING: Year Check
    const rmYear = new Date(date).getFullYear();
    if (rmYear !== APP_YEAR) {
        showNotification(`Matière enregistrée en ${rmYear}, visible uniquement en changeant l'exercice (Année actuelle: ${APP_YEAR}).`, 'warn');
    }

    renderRawMaterials();
    showNotification('Matière première enregistrée avec succès', 'success');
}
...
function editRawMaterial(id) {
    ...
    if (!rm) return showNotification('Matière introuvable', 'error');

    document.getElementById('raw-material-id').value = rm.id;
    document.getElementById('raw-material-date').value = new Date(rm.date).toISOString().slice(0, 10);
    document.getElementById('raw-material-product-name').value = rm.productName;
    document.getElementById('raw-material-arrived-qty').value = rm.arrivedQty;
    document.getElementById('raw-material-out-qty').value = rm.outQty;
    document.getElementById('raw-material-final-stock').value = rm.finalStock;
    document.getElementById('raw-material-raw-quantity').value = rm.rawQuantity || '';
    document.getElementById('raw-material-final-quantity').value = rm.finalQuantity || '';
    document.getElementById('raw-material-total-weight').value = rm.totalWeight || '';
    document.getElementById('raw-material-description').value = rm.description || '';

    const submitBtn = document.getElementById('raw-material-submit-btn');
    if (submitBtn) submitBtn.innerHTML = '<i class="fas fa-save"></i> Sauvegarder';

    setTimeout(() => document.getElementById('raw-material-product-name').focus(), 120);
}

window.renderRawMaterials = function () {
    const filterVal = document.getElementById('rawMaterialFilter')?.value || 'all';
    let rawMaterialsList = filterByTime(rawMaterials, filterVal).sort((a, b) => new Date(b.date) - new Date(a.date));

    const tbody = document.getElementById('rawMaterialTableBody');
    if (!tbody) return;

    if (rawMaterialsList.length === 0) {
        tbody.innerHTML = '<tr class="hover:bg-gray-50 dark:hover:bg-gray-700"><td colspan="11" class="px-2 py-2 text-center text-gray-500">Aucune matière enregistrée</td></tr>';
        return;
    }

    tbody.innerHTML = rawMaterialsList.map(rm => {
        const rmDate = new Date(rm.date);
        const dateStr = rmDate.toLocaleDateString('fr-FR');
        const timeStr = rmDate.toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' });

        return `
            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer">
                <td class="px-2 py-2" data-label="Date">${dateStr}</td>
                <td class="px-2 py-2" data-label="Heure">${timeStr}</td>
                <td class="px-2 py-2 font-medium" data-label="Produit">${rm.productName}</td>
                <td class="px-2 py-2 text-center" data-label="Arrivée (sacs)">${rm.arrivedQty.toFixed(2)}</td>
                <td class="px-2 py-2 text-center" data-label="Sortie (sacs)">${rm.outQty.toFixed(2)}</td>
                <td class="px-2 py-2 text-center" data-label="Stock Final (sacs)">${rm.finalStock.toFixed(2)}</td>
                <td class="px-2 py-2 text-center" data-label="Matière (kg)">${rm.rawQuantity ? rm.rawQuantity.toFixed(2) : '-'}</td>
                <td class="px-2 py-2 text-center" data-label="Produit (paquets)">${rm.finalQuantity ? rm.finalQuantity.toFixed(2) : '-'}</td>
                <td class="px-2 py-2 text-center" data-label="Poids Total">${rm.totalWeight ? rm.totalWeight.toFixed(2) : '-'}</td>
                <td class="px-2 py-2 truncate" data-label="Notes">${rm.description || '-'}</td>
                <td class="px-2 py-2 text-center" data-label="Actions">
                    <button onclick="editRawMaterial(${rm.id})" class="text-blue-500 hover:text-blue-700 mr-2"><i class="fas fa-edit"></i></button>
                    <button onclick="deleteRawMaterial(${rm.id})" class="text-red-400 hover:text-red-600"><i class="fas fa-trash-alt"></i></button>
                </td>
            </tr>
        `;
    }).join('');
...
window.exportRawMaterialPDF = function () {
    const filterVal = document.getElementById('rawMaterialFilter')?.value || 'all';
    let rawMaterialsList = filterByTime(rawMaterials, filterVal).sort((a, b) => new Date(b.date) - new Date(a.date));

    if (rawMaterialsList.length === 0) {
        return showNotification('Aucune matière à exporter', 'warn');
    }

    // Calculer les totaux par produit (arrivée - sortie = stock net)
    const productTotals = {};
    rawMaterialsList.forEach(rm => {
        const normalizedName = rm.productName ? rm.productName.trim() : '';
        if (!productTotals[normalizedName]) {
            productTotals[normalizedName] = {
                arrivedQty: 0,
                outQty: 0,
        ...
        productTotals[normalizedName].arrivedQty += rm.arrivedQty;
        productTotals[normalizedName].outQty += rm.outQty;
        productTotals[normalizedName].netStock = productTotals[normalizedName].arrivedQty - productTotals[normalizedName].outQty;
        productTotals[normalizedName].rawQuantity += (rm.rawQuantity || 0);
        productTotals[normalizedName].finalQuantity += (rm.finalQuantity || 0);
        productTotals[normalizedName].totalWeight += (rm.totalWeight || 0);
    });

    // Préparer les données du tableau
    const tableData = rawMaterialsList.map(rm => {
        const rmDate = new Date(rm.date);
        const dateStr = rmDate.toLocaleDateString('fr-FR');
        const timeStr = rmDate.toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' });
        return [
            ...
            timeStr,
            rm.productName,
            rm.arrivedQty.toFixed(2),
            rm.outQty.toFixed(2),
            rm.finalStock.toFixed(2),
            rm.rawQuantity ? rm.rawQuantity.toFixed(2) : '-',
            rm.finalQuantity ? rm.finalQuantity.toFixed(2) : '-',
            rm.totalWeight ? rm.totalWeight.toFixed(2) : '-',
            rm.description || '-'
        ];
    });
    ...
    const totalRows = uniqueProducts.map(prodName => {
        ...
        return [
            ...
            'TOTAL',
            prodName,
            totals.arrivedQty.toFixed(2),
            totals.outQty.toFixed(2),
            totals.netStock.toFixed(2),
            totals.rawQuantity ? totals.rawQuantity.toFixed(2) : '',
            totals.finalQuantity ? totals.finalQuantity.toFixed(2) : '',
            totals.totalWeight ? totals.totalWeight.toFixed(2) : '',
            ''
        ];
    });
    ...
    doc.setFontSize(18);
    doc.setTextColor(31, 122, 62);
    doc.text("RAPPORT DE GESTION MATIèRE PREMIèRE", pageWidth / 2, yPosition + 8, {
        align: 'center'
    });
    ...
    // RÉSUMÉ
    const totalArrivedQty = Object.values(productTotals).reduce((sum, t) => sum + t.arrivedQty, 0);
    const totalOutQty = Object.values(productTotals).reduce((sum, t) => sum + t.outQty, 0);
    const totalNetStock = totalArrivedQty - totalOutQty;
    const totalRawQty = Object.values(productTotals).reduce((sum, t) => sum + t.rawQuantity, 0);
    const totalFinalQty = Object.values(productTotals).reduce((sum, t) => sum + t.finalQuantity, 0);
    const totalWeightSum = Object.values(productTotals).reduce((sum, t) => sum + t.totalWeight, 0);
    ...
    doc.setTextColor(31, 122, 62);
    doc.setFont(undefined, 'bold');
    doc.text("RÉSUMÉ DES MATIÈRES PREMIÈRES", 20, finalY + 7);
    doc.setFontSize(9);
    doc.setTextColor(0);
    doc.setFont(undefined, 'normal');
    doc.text(`Total Arrivée: ${totalArrivedQty.toFixed(2)} sacs | Total Sortie: ${totalOutQty.toFixed(2)} sacs | Stock Net: ${totalNetStock.toFixed(2)} sacs`, 20, finalY + 13);
    doc.text(`Matière: ${totalRawQty.toFixed(2)} kg | Produit: ${totalFinalQty.toFixed(2)} paquets | Poids Total: ${totalWeightSum.toFixed(2)} kg`, 20, finalY + 19);
    ...
    const hfSuffix = getFilterFileSuffix(filterVal);
    doc.save(`Matieres_Premieres_${hfSuffix}_${new Date().toISOString().slice(0, 10)}.pdf`);
    showNotification('PDF exporté avec succès', 'success');
}

window.exportRawMaterialXLSX = function () {
    const filterVal = document.getElementById('rawMaterialFilter')?.value || 'all';
    let rawMaterialsList = filterByTime(rawMaterials, filterVal).sort((a, b) => new Date(b.date) - new Date(a.date));
    ...
    const sheetData = rawMaterialsList.map(rm => {
        const rmDate = new Date(rm.date);
        const dateStr = rmDate.toLocaleDateString('fr-FR');
        const timeStr = rmDate.toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' });
        return {
            'Date': dateStr,
            'Heure': timeStr,
            'Produit': rm.productName,
            'Arrivée (sacs)': rm.arrivedQty,
            'Sortie (sacs)': rm.outQty,
            'Stock Final (sacs)': rm.finalStock,
            'Matière (kg)': rm.rawQuantity || '',
            'Produit (paquets)': rm.finalQuantity || '',
            'Poids Total': rm.totalWeight || '',
            'Notes': rm.description || ''
        };
    });

    const ws = XLSX.utils.json_to_sheet(sheetData);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Matieres Premieres');
    XLSX.writeFile(wb, `Matieres_Premieres_${new Date().toISOString().split('T')[0]}.xlsx`);
    showNotification('Excel exporté avec succès', 'success');
}
...
function resetAllData() {
    ...
    if (confirm("ATTENTION: Vous allez effacer TOUTES les données (stock, ventes, dépenses, clients, devis, livraisons) de façon permanente. Êtes-vous sûr ?")) {
        ...
        if (typeof dbQuotes !== 'undefined') dbQuotes.clear().catch(err => console.warn('Error clearing quotes/deliveries:', err));
...
function exportStateJSON() {
    const state = {
        version: '3.0', // Version 3.0: support des productions et matières premières
        ...
        clients,
        suppliers,
        quotes, // Contient maintenant les devis ET les livraisons (type: 'quote' | 'delivery')
        companyInfo,
        logo: localStorage.getItem(LOGO_KEY),
        theme: localStorage.getItem(THEME_KEY),
        themeColor: localStorage.getItem(THEME_COLOR_KEY)
...
window.importStateJSON = function (input) {
    ...
    if (!confirm("ATTENTION : Cette action va effacer les données actuelles et les remplacer par celles du fichier de sauvegarde.\n\nEnsuite, une synchronisation vers Firebase sera forcée pour mettre à jour le cloud.\n\nVoulez-vous continuer ?")) {
    ...
    reader.onload = async function (e) {
        try {
            ...
            // Support des trois versions (1.0 ancien format, 2.0+ avec livraisons, 3.0+ avec productions et matières premières)
            const version = state.version || '1.0';
            console.log('Importing backup version:', version);

            if (typeof showToast === 'function') showToast("Restauration et Synchronisation en cours...", "info");

            // --- 1. NETTOYAGE (Dexie) ---
            // Utilisation directe de 'db' pour éviter les problèmes de portée avec les wrappers
            if (typeof db !== 'undefined') {
                ...
                // rhAppData n'est pas touché sauf si inclus dans le JSON (non standard)
            } else {
                console.warn("Mise en garde: Objet 'db' Dexie non trouvé. Le cache local ne sera pas mis à jour correctement.");
            }

            // --- 2. RESTAURATION (localStorage + Dexie) ---

            const restoreTable = async (key, tableName, data) => {
                if (data) {
                    localStorage.setItem(key, JSON.stringify(data));
                    if (typeof db !== 'undefined' && db[tableName]) {
                        await db[tableName].bulkPut(data);
                    }
                }
            };

            await restoreTable(INVENTORY_KEY, 'inventory', state.inventory);
            await restoreTable(DAILY_RECORDS_KEY, 'dailyRecords', state.dailyRecords);
            await restoreTable(EXPENSES_KEY, 'expenses', state.expenses);
            await restoreTable(INCOME_KEY, 'income', state.income);
            await restoreTable(PRODUCTION_KEY, 'productions', state.productions);
            await restoreTable(RAW_MATERIALS_KEY, 'rawMaterials', state.rawMaterials);
            await restoreTable(CLIENTS_KEY, 'clients', state.clients);
            await restoreTable(SUPPLIERS_KEY, 'suppliers', state.suppliers);
            await restoreTable(QUOTES_KEY, 'quotes', state.quotes); // Quotes & Deliveries
            ...
            // Hydrate in-memory state so the following saveData()/syncUp() uses the imported data
            if (Array.isArray(state.inventory)) inventory = state.inventory;
            if (Array.isArray(state.dailyRecords)) dailyRecords = state.dailyRecords;
            if (Array.isArray(state.expenses)) expenses = state.expenses;
            if (Array.isArray(state.income)) income = state.income;
            ...
            if (state.employees || state.attendance || state.payrollExtras || state.visibleSundays || state.meta) {
                localStorage.setItem('rh_app_data', JSON.stringify({
                    employees: state.employees || [],
                    attendance: state.attendance || {},
                    payrollExtras: state.payrollExtras || {},
                    visibleSundays: state.visibleSundays || [],
                    meta: state.meta || {}
                }));
            }

            // --- 3. FORCE SYNC UP (Backup to Cloud) ---
            if (window.GoogleSheetsAdapter && typeof window.GoogleSheetsAdapter.syncUp === 'function') {
                try {
                    console.log("Forcing Firebase Sync (Backup) after JSON restore...");
                    const syncResult = await window.GoogleSheetsAdapter.syncUp();
                    console.log("Firebase Sync successful:", syncResult);
                    alert("Restauration locale et Synchronisation Cloud terminées avec succès !\n\nLa page va se recharger.");
                } catch (syncErr) {
                    console.error("Firebase Sync failed after restore:", syncErr);
                    alert("ATTENTION: Restauration locale réussie, mais échec de la mise à jour du Cloud.\n\nErreur: " + syncErr.message + "\n\nVos données locales sont à jour, mais le Cloud contient encore les anciennes données. Essayez de synchroniser manuellement.");
                }
            } else {
                alert("Restauration locale réussie ! (Sync Cloud non disponible)\n\nLa page va se recharger.");
            }
...
async function resetBilan() {
    ...
    if (!confirm('ATTENTION: Vous allez supprimer l\'historique des ventes ET des livraisons (bilan) uniquement. Continuer ?')) return;

    dailyRecords = [];
    quotes = []; // Réinitialise aussi les livraisons (type: 'delivery')
    ...
    try {
        ...
        if (cfg.enabled) {
            const state = {
                ...
                expenses,
                clients,
                quotes: [], // Ventes, Devis ET Livraisons
                companyInfo,
                logo: localStorage.getItem(LOGO_KEY),
                theme: localStorage.getItem(THEME_KEY),
                themeColor: localStorage.getItem(THEME_COLOR_KEY)
    ...
    saveData();
    renderDailyHistory();
    renderQuotes(); // Affiche les livraisons aussi
    updateCharts();
    updateStats();
    alert('Bilan réinitialisé définitivement (ventes, devis et livraisons).');
}
...
function renderQuotes() {
    // Alias for backward compatibility - now renders deliveries
...
function renderDeliveries() {
    ...
    const sortedDeliveries = yearFiltered.sort((a, b) => new Date(b.date) - new Date(a.date));

    if (sortedDeliveries.length === 0) {
        tbody.innerHTML = `<tr><td colspan="7" class="text-center py-4 text-gray-400 italic">Aucune livraison enregistrée.</td></tr>`;
        return;
    }

    sortedDeliveries.forEach(delivery => {
        // Each delivery can have multiple items - create a row for each item
        const deliveryDate = new Date(delivery.date).toLocaleDateString('fr-FR');

        delivery.items.forEach((item, itemIdx) => {
            const tr = document.createElement('tr');
            const isFirstItem = itemIdx === 0;

            tr.className = "hover:bg-green-50 dark:hover:bg-gray-700 transition-colors border-b text-xs";
            tr.innerHTML = `
                        <td class="p-2 font-medium text-gray-700 dark:text-gray-200" data-label="Date">${isFirstItem ? deliveryDate : ''}</td>
                        <td class="p-2 font-semibold text-gray-800 dark:text-gray-200" data-label="Produit">${item.name}</td>
                        <td class="p-2 text-center font-mono text-gray-600 dark:text-gray-300" data-label="Stock Initial">${item.initialStock}</td>
                        <td class="p-2 text-center font-mono font-bold text-brand" data-label="Qté Livrée">${item.qty}</td>
                        <td class="p-2 text-center font-mono ${item.finalStock < 0 ? 'text-red-600 font-bold' : 'text-gray-600 dark:text-gray-300'}" data-label="Stock Final">${item.finalStock}</td>
                        <td class="p-2 text-gray-600 dark:text-gray-300" data-label="Client">${isFirstItem ? (delivery.clientName || 'N/A') : ''}</td>
                        <td class="p-2 flex gap-2 justify-center" data-label="Actions">${isFirstItem ? `
                            <button onclick="prepareInvoice(${delivery.id}, 'delivery')" class="text-blue-500 hover:text-blue-700 dark:text-blue-400" title="Bon de Livraison"><i class="fas fa-file-pdf"></i></button>
                            <button onclick="deleteQuoteRecord(${delivery.id})" class="text-red-500 hover:text-red-700 dark:text-red-400" title="Supprimer"><i class="fas fa-trash-alt"></i></button>
                        ` : ''}</td>
                    `;
            tbody.appendChild(tr);
        });
...
function handleDownloadPDF() {
    ...
    } else if (currentInvoiceData && currentInvoiceData.type === 'delivery') {
        generateDeliveryPDF();
    } else {
        generateInvoicePDF();
    }
...
function prepareInvoice(recordId, type) {
    ...
    if (type === 'sale') {
        docType = 'Facture';
        docRef = `FAC-${r.id.toString().slice(-6)}`;
    } else if (type === 'delivery') {
        docType = 'Bon de Livraison';
        docRef = `BON-${r.id.toString().slice(-6)}`;
    } else {
        docType = 'Devis';
        docRef = `DEV-${r.id.toString().slice(-6)}`;
    }
    ...
    (currentInvoiceData.items || []).forEach((item, index) => {
        ...
        // For deliveries, show different columns
        if (type === 'delivery') {
            tr.innerHTML = `
                        ...
                        <td class="text-center font-mono">${item.initialStock}</td>
                        <td class="text-center font-mono">${item.qty} ${item.unit || 'Unit'}</td>
                        <td class="text-center font-mono">${item.finalStock}</td>
    ...
    // Show Save button for sales (enable edit) and hide for quotes/deliveries; show PDF label accordingly
    const saveBtn = document.getElementById('invoice-save-btn');
    if (saveBtn) {
        if (type === 'sale') {
...
window.generateDeliveryPDF = function () {
    if (!currentInvoiceData) return alert("Aucune donnée de livraison é imprimer.");
    ...
    const bonRef = document.getElementById('invoice-ref').value;
    const bonDate = document.getElementById('invoice-date').value;
    ...
    // TITRE PRINCIPAL (Bon de Livraison)
    doc.setFontSize(28);
    doc.setTextColor(31, 122, 62);
    doc.setFont(undefined, 'bold');
    doc.text("BON DE LIVRAISON", pageWidth / 2, y + 12, {
        align: 'center'
    });

    // RÉFÉRENCE & DATE (en bas à droite de l'en-tête)
    doc.setFontSize(10);
    doc.setTextColor(0);
    doc.setFont(undefined, 'bold');
    doc.text(`Réf: ${bonRef}`, pageWidth - 15, y + 5, {
        align: 'right'
    });
    ...
    doc.setTextColor(100);
    doc.setFont(undefined, 'normal');
    doc.text(`Date: ${new Date(bonDate).toLocaleDateString('fr-FR')}`, pageWidth - 15, y + 12, {
        align: 'right'
    });
    ...
    doc.rect(15, clientBoxY, pageWidth - 30, 25, 'F');

    doc.setFontSize(9);
    doc.setTextColor(31, 122, 62);
    doc.setFont(undefined, 'bold');
    doc.text("CLIENT / DESTINATAIRE", 20, clientBoxY + 4);
    ...
    doc.setFontSize(9);
    doc.setFont(undefined, 'normal');
    doc.text(`Tél: ${clientPhone}`, 20, clientBoxY + 17);

    // ===== TABLEAU DÉTAILS LIVRAISON =====
    const tableStartY = clientBoxY + 30;
    ...
    doc.autoTable({
        startY: tableStartY,
        head: [
            ['#', 'Produit', 'Stock Initial', 'Qté Livrée', 'Stock Final', 'Unité']
        ],
        body: body,
        theme: 'grid',
        headStyles: {
    ...
    doc.text("- Tout problème doit être signalé dans les 48 heures", 15, notesY + 10);
    ...
    // Signature livreur (gauche)
    doc.text("Signature Livreur", 35, sigY + 8, {
        align: 'center'
    });
    doc.setDrawColor(200);
    doc.line(15, sigY + 15, 55, sigY + 15);

    // Signature Client (droite)
    doc.text("Signature Client", pageWidth - 35, sigY + 8, {
        align: 'center'
    });
    doc.line(pageWidth - 55, sigY + 15, pageWidth - 15, sigY + 15);

    // Cachet (centre)
    doc.text("Cachet", pageWidth / 2, sigY + 8, {
        align: 'center'
    });
    ...
    doc.setTextColor(150);
    doc.setFont(undefined, 'normal');
    doc.text(companyInfo.footer, pageWidth / 2, pageHeight - 8, {
        align: 'center'
    });
...
// --- FONCTIONS DE SYNCHRONISATION GOOGLE SHEETS ---
...
function openSyncModal() {
    ...
    if (modal) {
        ...
        // Pré-remplir la config si elle existe
        if (typeof GoogleSheetsAdapter !== 'undefined') {
            const savedUrl = GoogleSheetsAdapter.getApiUrl();
        ...
        if (statusDiv) {
            statusDiv.className = 'hidden mb-4 p-3 rounded text-sm';
            statusDiv.innerHTML = '';
        }
    }
}
...
async function handleSyncUp() {
    // Force refresh Dexie from current state before syncing to ensure cloud gets latest data
    if (typeof saveData === 'function') {
        try {
            await saveData();
        } catch (e) {
            console.warn('Pre-sync save failed', e);
        }
    }
    ...
    const hasFirebaseConfig = typeof window.FIREBASE_CONFIG !== 'undefined' && !!window.FIREBASE_CONFIG;

    if (!adapter || typeof adapter.syncUp !== 'function') {
        alert('Erreur: adaptateur de synchronisation non chargé.');
        return;
    }

    if (urlInput && urlInput.value.trim()) {
        if (typeof adapter.setApiUrl === 'function') {
            adapter.setApiUrl(urlInput.value.trim());
        }
    } else if (!hasFirebaseConfig) {
        alert('Veuillez configurer Firebase ou renseigner une URL de synchronisation.');
        return;
    }

    // UI Loading
    if (statusDiv) {
        statusDiv.className = 'block mb-4 p-3 rounded text-sm bg-blue-100 text-blue-800';
        statusDiv.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Sauvegarde en cours vers Firebase...';
    }

    try {
        const result = await adapter.syncUp();
        if (statusDiv) {
            statusDiv.className = 'block mb-4 p-3 rounded text-sm bg-green-100 text-green-800';
            statusDiv.innerHTML = '✅ Sauvegarde réussie ! ' + (result.timestamp ? `(${new Date(result.timestamp).toLocaleTimeString()})` : '');
        }
        if (window.showToast) showToast('Backup Cloud réussi !', 'success');
    } catch (e) {
        if (statusDiv) {
            statusDiv.className = 'block mb-4 p-3 rounded text-sm bg-red-100 text-red-800';
            statusDiv.innerHTML = '❌ Erreur: ' + e.message;
        }
        console.error(e);
        if (window.showToast) showToast('Erreur Backup: ' + e.message, 'error');
    }
}
...
async function handleSyncDown() {
    ...
    if (statusDiv) {
        statusDiv.className = 'block mb-4 p-3 rounded text-sm bg-blue-100 text-blue-800';
        statusDiv.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Restauration depuis Firebase...';
    }

    try {
        if (!confirm('ATTENTION: Cela va écraser vos données locales avec celles du Cloud. Continuer ?')) {
            if (statusDiv) statusDiv.innerHTML = '';
            return;
        }

        // Show clearer loading state as this might take a while
        if (statusDiv) statusDiv.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Téléchargement des données...';

        const result = await adapter.syncDown();
        if (statusDiv) {
            statusDiv.className = 'block mb-4 p-3 rounded text-sm bg-green-100 text-green-800';
            statusDiv.innerHTML = '✅ Restauration réussie ! La page va se recharger...';
        }
    } catch (e) {
        if (statusDiv) {
            statusDiv.className = 'block mb-4 p-3 rounded text-sm bg-red-100 text-red-800';
        ...
        console.error(e);
        if (window.showToast) showToast('Erreur Restore: ' + e.message, 'error');
    }
}

// Debug RH: list local sources, firebase info, and attempt PushRhNow
window.handleDebugRh = async function () {
    const statusDiv = document.getElementById('syncStatus');
    if (statusDiv) {
        statusDiv.className = 'block mb-4 p-3 rounded text-sm bg-blue-50 text-blue-800';
        statusDiv.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Diagnostic RH en cours...';
    }

    if (typeof window.ListRhLocal !== 'function' || typeof window.FirebaseSyncDebug !== 'function' || typeof window.PushRhNow !== 'function') {
        if (statusDiv) {
            statusDiv.className = 'block mb-4 p-3 rounded text-sm bg-red-100 text-red-800';
            statusDiv.innerHTML = '❌ Diagnostic non disponible: fonctions manquantes.';
        }
        return;
    }

    try {
        const local = await window.ListRhLocal();
        const fb = await window.FirebaseSyncDebug();
        let pushResult = null;
        try {
            pushResult = await window.PushRhNow();
        } catch (pushErr) {
            pushResult = { error: String(pushErr && pushErr.message || pushErr) };
        }

        const html = `
            <div style="max-height:260px;overflow:auto;font-size:0.85rem">
                <b>Firebase</b>: ${JSON.stringify(fb)}<br/>
                <b>LocalStorage.rh_app_data</b>: ${local.localStorageRaw ? 'present' : 'absent'}<br/>
                <b>dbRhData.get</b>: ${local.dbRhDataGet ? 'present' : 'absent'}<br/>
                <b>PushRhNow</b>: ${pushResult && pushResult.ok ? 'ok' : (pushResult && pushResult.fallback ? 'ok (fallback)' : ('error: ' + (pushResult && pushResult.error ? pushResult.error : JSON.stringify(pushResult))))}
            </div>`;

        if (statusDiv) {
            statusDiv.className = 'block mb-4 p-3 rounded text-sm bg-green-50 text-green-800';
            statusDiv.innerHTML = html;
        }
    } catch (err) {
        if (statusDiv) {
            statusDiv.className = 'block mb-4 p-3 rounded text-sm bg-red-100 text-red-800';
            statusDiv.innerHTML = '❌ Erreur diagnostic: ' + (err && err.message ? err.message : String(err));
        }
        console.error('handleDebugRh error', err);
    }
};
...
window.toggleRapportDate = function () {
    const periodSelect = document.getElementById('rapport-period');
...
// Helper: Get data and title based on selected period
function getReportData() {
    const periodSelect = document.getElementById('rapport-period');
    const period = periodSelect ? periodSelect.value : 'day';
    const dateInput = document.getElementById('rapport-date');
    const dateStr = (period === 'day' && dateInput) ? dateInput.value : new Date().toISOString().split('T')[0];

    // Default fallback if date not selected
    if (period === 'day' && !dateStr) return null;
    ...
    let filterFn = null;
    if (period === 'day') {
        filterFn = (item) => (item.date || '').substring(0, 10) === dateStr;
        title = "RAPPORT DU JOUR";
        displayPeriod = new Date(dateStr).toLocaleDateString('fr-FR', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
    } else {
        const threshold = getDateThreshold(period);
        filterFn = (item) => new Date(item.date) >= threshold;
        if (period === 'week') { title = "RAPPORT HEBDOMADAIRE"; displayPeriod = "Semaine en cours"; }
        else if (period === 'month') { title = "RAPPORT MENSUEL"; displayPeriod = "Mois en cours"; }
        else if (period === 'year') { title = "RAPPORT ANNUEL"; displayPeriod = "Année en cours"; }
    }

    // Load Data
    const dailyRecords = JSON.parse(localStorage.getItem(DAILY_RECORDS_KEY) || '[]');
    const expenses = JSON.parse(localStorage.getItem(EXPENSES_KEY) || '[]');
    const income = JSON.parse(localStorage.getItem(INCOME_KEY) || '[]');
    const productions = JSON.parse(localStorage.getItem(PRODUCTION_KEY) || '[]');
    const rawMaterials = JSON.parse(localStorage.getItem(RAW_MATERIALS_KEY) || '[]');

    // Apply Filter
    const filterArr = (arr) => arr.filter(filterFn);

    return {
        sales: filterArr(dailyRecords),
        expenses: filterArr(expenses),
        income: filterArr(income),
        productions: filterArr(productions),
        rawMaterials: filterArr(rawMaterials),
        title,
        displayPeriod,
        period,
        dateStr
...
function renderDailyReport() {
    ...
    const container = document.getElementById('rapport-dashboard');
    if (!container) return;

    if (!data) return; // e.g. no date selected

    // Prepare totals
    let salesTotal = 0; data.sales.forEach(s => { salesTotal += parseFloat(s.total || s.amount || 0); });
    let expensesTotal = 0; data.expenses.forEach(e => { expensesTotal += parseFloat(e.amount || 0); });
    let incomeTotal = 0; data.income.forEach(i => { incomeTotal += parseFloat(i.amount || 0); });

    let prodRawTotal = 0, prodFinalTotal = 0, prodWeightTotal = 0;
    data.productions.forEach(p => {
        prodRawTotal += parseFloat(p.rawQuantity || 0);
        prodFinalTotal += parseFloat(p.finalQuantity || 0);
        prodWeightTotal += parseFloat(p.totalWeight || 0);
    });

    let rmArrivedTotal = 0, rmOutTotal = 0;
    data.rawMaterials.forEach(rm => {
        rmArrivedTotal += parseFloat(rm.arrivedQty || 0);
        rmOutTotal += parseFloat(rm.outQty || 0);
    });
    const rmNetStock = rmArrivedTotal - rmOutTotal;

    const formatNum = (n) => n.toFixed(2).replace('.', ',').replace(/\B(?=(\d{3})+(?!\d))/g, " ");

    container.innerHTML = `
        <div class="col-span-full mb-2">
            <p class="text-sm text-gray-500 dark:text-gray-400 font-medium uppercase tracking-wide">
                <i class="fas fa-calendar-day mr-1"></i> ${data.displayPeriod}
            </p>
        </div>

        <div class="m3-card p-5 hover:shadow-lg transition-all">
            <div class="flex items-center gap-3 mb-3">
                <div class="w-10 h-10 rounded-full bg-green-100 dark:bg-green-900 flex items-center justify-center">
                    <i class="fas fa-shopping-cart text-green-600 dark:text-green-300"></i>
                </div>
                <h3 class="font-bold text-gray-800 dark:text-gray-100">Ventes</h3>
            </div>
            <div class="space-y-1 text-sm">
                <p class="text-gray-600 dark:text-gray-300">Nombre: <strong>${data.sales.length}</strong></p>
                <p class="text-gray-600 dark:text-gray-300">CA Total: <strong class="text-green-700 dark:text-green-400">${formatNum(salesTotal)} F</strong></p>
            </div>
        </div>
        ...
        <div class="m3-card p-5 hover:shadow-lg transition-all">
            <div class="flex items-center gap-3 mb-3">
                ...
                <h3 class="font-bold text-gray-800 dark:text-gray-100">Dépenses</h3>
            </div>
            <div class="space-y-1 text-sm">
                <p class="text-gray-600 dark:text-gray-300">Nombre: <strong>${data.expenses.length}</strong></p>
                <p class="text-gray-600 dark:text-gray-300">Total: <strong class="text-red-700 dark:text-red-400">${formatNum(expensesTotal)} F</strong></p>
            </div>
        </div>
        ...
        <div class="m3-card p-5 hover:shadow-lg transition-all">
            <div class="flex items-center gap-3 mb-3">
                <div class="w-10 h-10 rounded-full bg-blue-100 dark:bg-blue-900 flex items-center justify-center">
                    <i class="fas fa-arrow-up text-blue-600 dark:text-blue-300"></i>
                </div>
                <h3 class="font-bold text-gray-800 dark:text-gray-100">Rentrées</h3>
            </div>
            <div class="space-y-1 text-sm">
                <p class="text-gray-600 dark:text-gray-300">Nombre: <strong>${data.income.length}</strong></p>
                <p class="text-gray-600 dark:text-gray-300">Total: <strong class="text-blue-700 dark:text-blue-400">${formatNum(incomeTotal)} F</strong></p>
            </div>
        </div>
        ...
        <div class="m3-card p-5 hover:shadow-lg transition-all">
            <div class="flex items-center gap-3 mb-3">
                <div class="w-10 h-10 rounded-full bg-purple-100 dark:bg-purple-900 flex items-center justify-center">
                    <i class="fas fa-industry text-purple-600 dark:text-purple-300"></i>
                </div>
                <h3 class="font-bold text-gray-800 dark:text-gray-100">Production</h3>
            </div>
            <div class="space-y-1 text-sm">
                <p class="text-gray-600 dark:text-gray-300">Nombre: <strong>${data.productions.length}</strong></p>
                <p class="text-gray-600 dark:text-gray-300">Matière: <strong>${formatNum(prodRawTotal)} kg</strong></p>
                <p class="text-gray-600 dark:text-gray-300">Produit: <strong>${formatNum(prodFinalTotal)} paquets</strong></p>
                <p class="text-gray-600 dark:text-gray-300">Poids total: <strong>${formatNum(prodWeightTotal)} kg</strong></p>
            </div>
        </div>
        ...
        <div class="m3-card p-5 hover:shadow-lg transition-all">
            <div class="flex items-center gap-3 mb-3">
                <div class="w-10 h-10 rounded-full bg-orange-100 dark:bg-orange-900 flex items-center justify-center">
                    <i class="fas fa-boxes text-orange-600 dark:text-orange-300"></i>
                </div>
                <h3 class="font-bold text-gray-800 dark:text-gray-100">Matière Première</h3>
            </div>
            <div class="space-y-1 text-sm">
                <p class="text-gray-600 dark:text-gray-300">Mouvements: <strong>${data.rawMaterials.length}</strong></p>
                <p class="text-gray-600 dark:text-gray-300">Arrivée: <strong class="text-green-700 dark:text-green-400">${formatNum(rmArrivedTotal)} sacs</strong></p>
                <p class="text-gray-600 dark:text-gray-300">Sortie: <strong class="text-red-700 dark:text-red-400">${formatNum(rmOutTotal)} sacs</strong></p>
                <p class="text-gray-600 dark:text-gray-300">Stock Net: <strong>${formatNum(rmNetStock)} sacs</strong></p>
            </div>
        </div>

        <div class="m3-card p-5 hover:shadow-lg transition-all col-span-full sm:col-span-2 lg:col-span-3">
            <div class="flex items-center gap-3 mb-3">
                <div class="w-10 h-10 rounded-full bg-emerald-100 dark:bg-emerald-900 flex items-center justify-center">
                    <i class="fas fa-chart-line text-emerald-600 dark:text-emerald-300"></i>
                </div>
                <h3 class="font-bold text-gray-800 dark:text-gray-100 text-lg">Bilan (${data.displayPeriod})</h3>
            </div>
            <div class="flex flex-wrap gap-6 text-sm">
                <p class="text-gray-600 dark:text-gray-300">Entrées: <strong class="text-green-700 dark:text-green-400 text-base">${formatNum(salesTotal + incomeTotal)} F</strong></p>
                <p class="text-gray-600 dark:text-gray-300">Sorties: <strong class="text-red-700 dark:text-red-400 text-base">${formatNum(expensesTotal)} F</strong></p>
                <p class="text-gray-600 dark:text-gray-300">Solde: <strong class="${(salesTotal + incomeTotal - expensesTotal) >= 0 ? 'text-green-700 dark:text-green-400' : 'text-red-700 dark:text-red-400'} text-base">${formatNum(salesTotal + incomeTotal - expensesTotal)} F</strong></p>
            </div>
        </div>
    `;
}
...
function exportDailyReportPDF() {
    const data = getReportData();
    if (!data) { if (window.showToast) showToast('Aucune donnée à exporter', 'error'); return; }

    const formatNum = (n) => n.toFixed(2).replace('.', ',').replace(/\B(?=(\d{3})+(?!\d))/g, " ");

    // Calculate totals again (or export data function could return totals)
    // Redoing loop is cheap
    let salesTotal = 0; data.sales.forEach(s => { salesTotal += parseFloat(s.total || s.amount || 0); });
    let expensesTotal = 0; data.expenses.forEach(e => { expensesTotal += parseFloat(e.amount || 0); });
    let incomeTotal = 0; data.income.forEach(i => { incomeTotal += parseFloat(i.amount || 0); });
    ...
    const rmNetStock = rmArrivedTotal - rmOutTotal;
    const solde = salesTotal + incomeTotal - expensesTotal;
    ...
    // LOGO
    const logoData = localStorage.getItem(LOGO_KEY);
    if (logoData) { try { doc.addImage(logoData, 'JPEG', 15, y, 25, 25); } catch (e) { console.warn("Logo error", e); } }

    // EN-TÊTE
    doc.setFontSize(18);
    doc.setTextColor(31, 122, 62);
    doc.text(data.title, pageWidth / 2, y + 8, { align: 'center' });

    doc.setFontSize(11);
    doc.setTextColor(100);
    doc.text(data.displayPeriod, pageWidth / 2, y + 15, { align: 'center' });

    const company = JSON.parse(localStorage.getItem(COMPANY_KEY) || '{}');
    if (company.name) {
        doc.setFontSize(9);
        doc.setTextColor(120);
        doc.text([company.name, company.phone, company.address].filter(Boolean).join(' | '), pageWidth / 2, y + 21, { align: 'center' });
        y += 28;
    } else { y += 22; }

    doc.setDrawColor(31, 122, 62);
    ...
    // === TABLE ===
    const tableData = [
        ['Ventes', data.sales.length + ' opération(s)', formatNum(salesTotal) + ' F'],
        ['Dépenses', data.expenses.length + ' opération(s)', formatNum(expensesTotal) + ' F'],
        ["Rentrées d'argent", data.income.length + ' opération(s)', formatNum(incomeTotal) + ' F'],
        ['Production - Matière', data.productions.length + ' production(s)', formatNum(prodRawTotal) + ' kg'],
        ['Production - Produit', '', formatNum(prodFinalTotal) + ' paquets'],
        ['Production - Poids', '', formatNum(prodWeightTotal) + ' kg'],
        ['Matière 1ère - Arrivée', data.rawMaterials.length + ' mouvement(s)', formatNum(rmArrivedTotal) + ' sacs'],
        ['Matière 1ère - Sortie', '', formatNum(rmOutTotal) + ' sacs'],
        ['Matière 1ère - Stock Net', '', formatNum(rmNetStock) + ' sacs'],
    ];
    ...
    doc.autoTable({
        ...
        body: tableData,
        theme: 'grid',
        headStyles: { fillColor: [31, 122, 62], textColor: 255, fontStyle: 'bold', fontSize: 10, halign: 'center' },
        bodyStyles: { fontSize: 9, textColor: [50, 50, 50] },
        alternateRowStyles: { fillColor: [245, 250, 245] },
        columnStyles: { 0: { fontStyle: 'bold', cellWidth: 60 }, 1: { halign: 'center', cellWidth: 50 }, 2: { halign: 'right', fontStyle: 'bold' } },
    ...
    doc.setFontSize(12);
    doc.text(`BILAN (${data.displayPeriod})`, pageWidth / 2, finalY + 8, { align: 'center' });
    doc.setFontSize(10);
    doc.text('Entrées (Ventes + Rentrées): ' + formatNum(salesTotal + incomeTotal) + ' F', 20, finalY + 16);
    doc.text('Sorties (Dépenses): ' + formatNum(expensesTotal) + ' F', 20, finalY + 22);
    doc.text('Solde: ' + formatNum(solde) + ' F', pageWidth - 20, finalY + 19, { align: 'right' });

    finalY += 38;
    doc.setFontSize(8);
    doc.setTextColor(150);
    doc.text('Généré le ' + new Date().toLocaleDateString('fr-FR') + ' à ' + new Date().toLocaleTimeString('fr-FR'), pageWidth / 2, finalY, { align: 'center' });
    ...
    const filename = `Rapport_${data.period}_${data.dateStr || new Date().toISOString().split('T')[0]}.pdf`;
    doc.save(filename);
    if (window.showToast) showToast('Rapport PDF exporté avec succès!', 'success');
}
