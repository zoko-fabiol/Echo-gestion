...
const INVENTORY_KEY = 'stock_expert_inventory_v3';
        ...
        const GLASS_ENABLED_KEY = 'stock_expert_glass_enabled_stock_v1'; // Spécifique é stock.html
...
function showToast(message, type = 'success') {
    ...
    setTimeout(() => {
        ...
        window.tutorialStepsStock = [{
            title: 'Bienvenue',
            content: 'Bienvenue dans la gestion de stock é ce tutoriel couvre les actions courantes : produits, ventes, paramétres et sauvegarde.'
        }, {
            title: 'Navigation',
            content: 'Utilisez l\'en-téte et la barre latérale pour naviguer entre les sections Principales. Le bouton "Accueil" raméne é la gestion RH.'
        }, {
            title: 'Ajouter un produit',
            content: 'Cliquez sur le bouton + ou "Ajouter un produit" pour ouvrir le modal. Remplissez le nom, la catégorie, le stock et les prix, puis sauvegardez.'
        }, {
            title: 'Modifier / Supprimer',
            content: 'Pour modifier un produit, cliquez dessus dans la liste ou utilisez les actions (éditer / supprimer). Les changements sont sauvegardés en local.'
        }, {
            title: 'Filtrer & Rechercher',
            content: 'Utilisez le filtre de type (brut / fini) et la barre de recherche pour trouver rapidement un produit. Les filtres s\'appliquent é l\'export également.'
        }, {
            title: 'Enregistrer une vente',
            content: 'Ajoutez des articles au panier, vérifiez les quantités puis générez une vente. Les journaux quotidiens conservent l\'historique des ventes.'
        }, {
            title: 'Journaux & Rapports',
            content: 'Consultez les ventes journaliéres et exportez les rapports (XLSX / PDF) depuis les boutons d\'export.'
        }, {
            title: 'Paramétres & Logo',
            content: 'Ouvrez les paramétres pour modifier la couleur principale, le logo, le pied de page et l\'effet liquid glass.'
        }, {
            title: 'Sauvegarde automatique',
            content: 'Configurez le rappel de sauvegarde automatique et effectuez des exports/restaurations depuis "Sauvegarde & Restauration".'
        }, {
            title: 'Générer facture / devis',
            content: 'Utilisez l\'éditeur de facture pour créer un devis ou facture, puis téléchargez le PDF via le bouton de téléchargement.'
        }, {
            title: 'Fin',
            content: 'C\'est tout ! Explorez l\'application et réalisez quelques actions (ajout produit, vente, export) pour vous familiariser.'
        }];

        window._tutorialIndexStock = 0;
        window.renderTutorialStock = function() {
            const t = window.tutorialStepsStock[window._tutorialIndexStock];
            const titleEl = document.getElementById('tutorialTitleStock');
            const contentEl = document.getElementById('tutorialContentStock');
            if (titleEl) titleEl.innerText = (t ? t.title : '');
            if (contentEl) contentEl.innerText = (t ? t.content : '');
            const prev = document.getElementById('tutorialPrevStock');
            const next = document.getElementById('tutorialNextStock');
            if (prev) prev.style.display = window._tutorialIndexStock === 0 ? 'none' : 'inline-block';
            if (next) next.innerText = window._tutorialIndexStock === window.tutorialStepsStock.length - 1 ? 'Terminer' : 'Suivant';
        };

        window.startTutorialStock = function() {
            window._tutorialIndexStock = 0;
            window.renderTutorialStock();
            const modal = document.getElementById('tutorialModalStock');
            if (modal) modal.classList.remove('hidden');
        };

        window.closeTutorialStock = function() {
            const modal = document.getElementById('tutorialModalStock');
            if (modal) modal.classList.add('hidden');
        };

        window.nextTutorialStock = function() {
            if (window._tutorialIndexStock < window.tutorialStepsStock.length - 1) {
                window._tutorialIndexStock++;
                window.renderTutorialStock();
            } else {
                window.closeTutorialStock();
            }
        };

        window.prevTutorialStock = function() {
            if (window._tutorialIndexStock > 0) {
                window._tutorialIndexStock--;
                window.renderTutorialStock();
            }
        };
        ...
        let currentMode = 'sale'; // 'sale' ou 'quote'
        ...
        function initGlassSettings() {
            ...
            const opacityLight = 0.62 * (opacity / 100);
            const opacityDark = 0.04 * (opacity / 100);
            ...
            // Listen for glass settings changes from index.html
            try {
                if ('BroadcastChannel' in window) {
                    const bc = new BroadcastChannel('stock_expert_sync');
                    bc.addEventListener('message', (ev) => {
                        const msg = ev.data || {};
                        if (msg.type === 'glass-settings' && msg.opacity !== undefined) {
                            ...
                            if (msg.enabled !== undefined) {
                                localStorage.setItem(GLASS_ENABLED_KEY, msg.enabled ? 'true' : 'false');
                                if (msg.enabled) {
                            ...
                            localStorage.setItem('stock_expert_glass_opacity', opacity);
                            updateGlassUI();
                        }
                    });
                }
        ...
        function loadData() {
            ...
            const glassCheckbox = document.getElementById('settings-glass-enabled');
            if (glassCheckbox) glassCheckbox.checked = isGlassEnabled();
        ...
        function switchTab(tab) {
            ...
            document.querySelectorAll('[id^="tab-"]').forEach(el => {
                el.classList.remove('border-brand-green', 'text-brand', 'border-b-2');
                el.classList.add('text-gray-500', 'dark:text-gray-400');
            });

            document.getElementById(`content-${tab}`).classList.remove('hidden');
            const tabBtn = document.getElementById(`tab-${tab}`);
            tabBtn.classList.add('border-brand-green', 'text-brand', 'border-b-2');
        ...
        function switchSubTab(subtab) {
            ...
            document.querySelectorAll('[id$="-btn"]').forEach(el => {
                el.classList.remove('border-brand-green', 'text-brand', 'border-b-2');
                el.classList.add('text-gray-500', 'dark:text-gray-400', 'hover:text-brand');
            });

            document.getElementById(subtab).classList.remove('hidden');
            const subtabBtn = document.getElementById(`${subtab}-btn`);
            subtabBtn.classList.add('border-brand-green', 'text-brand', 'border-b-2');
            subtabBtn.classList.remove('text-gray-500', 'dark:text-gray-400', 'hover:text-brand');
        }
        ...
        function switchExpenseTab(tab) {
            ...
            if (tab === 'tab-depenses') {
                depensesBtn.classList.add('bg-brand', 'text-white');
                depensesBtn.classList.remove('border', 'border-gray-300', 'text-gray-700', 'dark:text-gray-300');
                rentréesBtn.classList.remove('bg-brand', 'text-white');
                rentréesBtn.classList.add('border', 'border-gray-300', 'text-gray-700', 'dark:text-gray-300');
            } else {
                rentréesBtn.classList.add('bg-brand', 'text-white');
                rentréesBtn.classList.remove('border', 'border-gray-300', 'text-gray-700', 'dark:text-gray-300');
                depensesBtn.classList.remove('bg-brand', 'text-white');
                depensesBtn.classList.add('border', 'border-gray-300', 'text-gray-700', 'dark:text-gray-300');
                renderIncome();
            }
        }
        ...
        function renderProductTiles() {
            ...
            Object.keys(grouped).sort().forEach(category => {
                ...
                grouped[category].sort((a, b) => a.name.localeCompare(b.name)).forEach(item => {
                    ...
                    div.className = 'product-tile glass interactive bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm text-center border dark:border-gray-700 min-h-[140px]';
        ...
        function toggleMode(mode) {
            ...
            document.getElementById('toggle-sale-btn').className = isSale ? 'flex-1 py-1.5 rounded text-xs font-semibold bg-brand text-white' : 'flex-1 py-1.5 rounded text-xs font-semibold text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700';
            document.getElementById('toggle-quote-btn').className = isSale ? 'flex-1 py-1.5 rounded text-xs font-semibold text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700' : 'flex-1 py-1.5 rounded text-xs font-semibold bg-brand text-white';

            // Mise é jour du titre
            document.getElementById('current-mode-title').innerText = isSale ? 'Saisie des ventes' : 'Création de Devis (Proforma)';

            // Mise é jour du bouton de validation
            document.getElementById('validate-btn-text').innerText = isSale ? 'Valider & Facturer (F10)' : 'Sauvegarder Devis (F10)';
            document.getElementById('validate-btn').classList.toggle('bg-brand', isSale);
            document.getElementById('validate-btn').classList.toggle('bg-blue-500', !isSale);
            document.getElementById('validate-btn').classList.toggle('hover:bg-brand-dark', isSale);
            document.getElementById('validate-btn').classList.toggle('hover:bg-blue-700', !isSale);
            ...
            // Mettre é jour le statut du panier si des éléments sont dedans
            if (cart.length > 0) renderCart();
        }

        // --- Refactorisation de la validation Vente/Devis ---
        function handleSaleValidation() {
            if (isReadOnlyUser()) {
                showToast("Accès en lecture seule : opération non autorisée.", "error");
                return;
            }
        ...
        function saveQuote() {
            ...
            const quoteRecord = {
                ...
                items: cart.map(i => ({
                    ...
                    qty: i.qty,
                    price: i.salePrice,
                    cost: i.purchasePrice, // Conserver l'info coét pour le calcul de marge si le devis est converti en vente
                    total: i.qty * i.salePrice,
                    totalCost: i.qty * i.purchasePrice
                })),
                total: totalQuote,
                clientName: document.getElementById('client-name').value || '', // Tentative de récupération client si modal ouverte
                clientPhone: document.getElementById('client-phone').value || '', // Tentative de récupération client si modal ouverte
            };
            ...
            renderCart();
            renderQuotes();
            prepareInvoice(quoteRecord.id, 'quote'); // Ouvrir le modal d'édition/facturation/devis
        }
        ...
        function renderInventory() {
            ...
            filteredProducts.sort((a, b) => a.name.localeCompare(b.name)).forEach(item => {
                ...
                tr.setAttribute('data-id', item.id);
                tr.innerHTML = `
                    <td class="px-2 py-2 font-medium"><div class="text-gray-900 dark:text-white truncate w-24 sm:w-32" title="${item.name}">${item.name}</div></td>
        ...
        function renderDailyHistory() {
            ...
            const sales = filterByTime(dailyRecords.filter(r => r.type === 'sale'), filter).sort((a, b) => new Date(b.date) - new Date(a.date));
            ...
            sales.forEach(r => {
                ...
                tr.innerHTML = `
                    <td class="p-2 font-medium text-gray-700 dark:text-gray-200">${date}</td>
                    <td class="p-2 text-gray-600 dark:text-gray-300">${time}</td>
                    <td class="p-2 font-semibold text-green-600 dark:text-green-400">${r.items.length} art.</td>
                    <td class="p-2 text-right font-bold text-green-600 dark:text-green-400">${r.total.toLocaleString()} FCFA</td>
                    <td class="p-2 text-right font-bold text-yellow-600 dark:text-yellow-400">${margin.toLocaleString()} FCFA</td>
                    <td class="p-2 text-gray-600 dark:text-gray-300 truncate" title="${r.items.map(i => i.name).join(', ')}">${r.items.map(i => i.name).join(', ')}</td>
        ...
        function renderIncome() {
            ...
            const filter = document.getElementById('incomeFilter') ? document.getElementById('incomeFilter').value : 'all';
            const list = filterByTime(income, filter).sort((a, b) => new Date(b.date) - new Date(a.date));
            ...
            list.forEach(inc => {
                ...
                row.innerHTML = `
                    <td class="p-2 text-xs font-medium text-gray-700 dark:text-gray-200">${date}</td>
                    <td class="p-2 text-xs text-gray-600 dark:text-gray-300">${time}</td>
                    <td class="p-2 text-xs font-semibold text-purple-600 dark:text-purple-400">${inc.receivedBy}</td>
                    <td class="p-2 text-xs text-gray-600 dark:text-gray-300">${inc.source || '-'}</td>
                    <td class="p-2 text-xs text-gray-600 dark:text-gray-300 max-w-xs truncate" title="${inc.description || '-'}">${inc.description || '-'}</td>
                    <td class="p-2 text-xs font-bold text-green-600 dark:text-green-400 text-right">${inc.amount.toLocaleString()} FCFA</td>
                    <td class="p-2 text-xs flex gap-2 justify-center">
        ...
        function renderExpenses() {
            ...
            let list = filterByTime(expenses, filter);
            ...
            list.forEach(exp => {
                ...
                tr.innerHTML = `
                    <td class="px-2 py-2 text-gray-500">${new Date(exp.date).toLocaleDateString('fr-FR')}</td>
                    <td class="px-2 py-2">
                        <div>${typeBadge}<span class="font-medium">${exp.category}</span></div>
                        ${detailsHtml}
                        ${transportHtml}${lossHtml}${paymentHtml}
                    </td>
                    <td class="px-2 py-2 text-right font-bold text-red-600">${exp.amount.toLocaleString()} F</td>
                    <td class="px-2 py-2 text-center whitespace-nowrap">${actionHtml}</td>
                `;
                tbody.appendChild(tr);
            });
        ...
        function saveSettings() {
            ...
            companyInfo.name = document.getElementById('settings-name').value;
            companyInfo.contact = document.getElementById('settings-contact').value;
            companyInfo.footer = document.getElementById('settings-footer').value;
            // Backend settings
            const backendEnabled = document.getElementById('settings-backend-enabled') ? document.getElementById('settings-backend-enabled').checked : false;
            const backendUrl = document.getElementById('settings-backend-url') ? document.getElementById('settings-backend-url').value.trim() : '';
            const backendToken = document.getElementById('settings-backend-token') ? document.getElementById('settings-backend-token').value.trim() : '';
            const backendAutoPush = document.getElementById('settings-backend-autopush') ? document.getElementById('settings-backend-autopush').checked : false;
            const backendConfig = {
                enabled: backendEnabled,
                baseUrl: backendUrl,
                token: backendToken,
                autoPush: backendAutoPush
            };
            setBackendConfig(backendConfig);
            const glassEnabled = document.getElementById('settings-glass-enabled') ? document.getElementById('settings-glass-enabled').checked : true;
        ...
        function updateStats() {
            const totalRevenue = dailyRecords.reduce((sum, r) => sum + r.total, 0);
            const totalCost = dailyRecords.reduce((sum, r) => sum + (r.totalCost || 0), 0);
            const totalExpenses = expenses.reduce((sum, e) => sum + e.amount, 0);
            const totalDebts = expenses.reduce((sum, e) => {
        ...
        function updateCharts() {
            if (!document.getElementById('chartRevenue')) return; // Check if the element exists

            const isDark = document.documentElement.classList.contains('dark');
            const textColor = isDark ? 'white' : 'black';
            const gridColor = isDark ? '#374151' : '#e5e7eb';
            const brandColor = getComputedStyle(document.documentElement).getPropertyValue('--brand-green').trim();
            ...
            dailyRecords.filter(r => r.type === 'sale').forEach(r => {
                const dateKey = r.date.slice(0, 10);
                if (daysData[dateKey]) {
                    daysData[dateKey].revenue += r.total;
                }
            });
            ...
            expenses.forEach(e => {
                ...
                if (daysData[dateKey]) {
                    daysData[dateKey].expenses += e.amount;
                }
            });

            const sortedData = Object.keys(daysData).sort().map(key => daysData[key]);
            const labels = sortedData.map(d => d.date);
            const revenues = sortedData.map(d => d.revenue);
            const expAmounts = sortedData.map(d => d.expenses);
            ...
            if (revenueChart) {
                ...
                revenueChart.data.datasets[0].borderColor = brandColor;
                revenueChart.data.datasets[1].data = expAmounts;
                revenueChart.data.datasets[1].borderColor = 'red';
                ...
                revenueChart = new Chart(ctx, {
                    ...
                    data: {
                        ...
                        datasets: [{
                            ...
                            backgroundColor: brandColor + '40', // 40 is opacity 25%
                            tension: 0.3,
                            fill: true,
                            pointRadius: 4,
                        }, {
        ...
        function setClientMode(mode) {
            ...
            document.getElementById('btn-mode-client').classList.toggle('bg-brand', mode === 'client');
            document.getElementById('btn-mode-client').classList.toggle('text-white', mode === 'client');
            document.getElementById('btn-mode-client').classList.toggle('border', mode !== 'client');
            document.getElementById('btn-mode-client').classList.toggle('border-gray-300', mode !== 'client');
            document.getElementById('btn-mode-client').classList.toggle('text-gray-700', mode !== 'client');
            document.getElementById('btn-mode-client').classList.toggle('dark:text-gray-300', mode !== 'client');

            document.getElementById('btn-mode-supplier').classList.toggle('bg-brand', mode === 'supplier');
            document.getElementById('btn-mode-supplier').classList.toggle('text-white', mode === 'supplier');
            document.getElementById('btn-mode-supplier').classList.toggle('border', mode !== 'supplier');
            document.getElementById('btn-mode-supplier').classList.toggle('border-gray-300', mode !== 'supplier');
            document.getElementById('btn-mode-supplier').classList.toggle('text-gray-700', mode !== 'supplier');
            document.getElementById('btn-mode-supplier').classList.toggle('dark:text-gray-300', mode !== 'supplier');
        ...
        function renderClientSupplierList() {
            ...
            sortedList.forEach((entry) => {
                ...
                tr.innerHTML = `
                    <td class="p-2 font-medium text-gray-700 dark:text-gray-200">${type}</td>
                    ...
                    <td class="p-2 text-gray-600 dark:text-gray-300">${entry.phone || '-'}</td>
                    <td class="p-2 text-gray-600 dark:text-gray-300 truncate" title="${entry.address || '-'}">${entry.address || '-'}</td>
                    <td class="p-2 text-gray-600 dark:text-gray-300 text-center text-xs">ID: ${entry.id}</td>
                    <td class="p-2 flex gap-2 justify-center">
        ...
        function renderQuotes() {
            ...
            sortedQuotes.forEach(q => {
                ...
                tr.innerHTML = `
                    <td class="p-2 font-medium text-gray-700 dark:text-gray-200">${date}</td>
                    <td class="p-2 text-gray-600 dark:text-gray-300">${time}</td>
                    <td class="p-2 font-semibold text-blue-600 dark:text-blue-400">${clientDisplay}</td>
                    <td class="p-2 text-right font-bold text-blue-600 dark:text-blue-400">${q.total.toLocaleString()} FCFA</td>
                    <td class="p-2 text-gray-600 dark:text-gray-300">${q.items ? q.items.length : 0} art.</td>
                    <td class="p-2 text-gray-600 dark:text-gray-300 truncate" title="${q.items ? q.items.map(i => i.name).join(', ') : '-'}">${q.items ? q.items.map(i => i.name).join(', ') : '-'}</td>
                    <td class="p-2 flex gap-2 justify-center">
                ...
                mutations.forEach(m => {
                    m.addedNodes.forEach(node => {
                        ...
                        try {
                            ...
                            ['.bg-white', '.bg-gray-50', '.rounded-xl', '.modal', '.card'].forEach(sel => {
                                if (node.matches && node.matches(sel)) node.classList.add('glass');
                                else if (node.querySelectorAll) node.querySelectorAll(sel).forEach(el => el.classList.add('glass'));
                            });

                            // Apply glass + interactive to controls
                            ['button', 'input', 'select', 'textarea'].forEach(sel => {
                                if (node.matches && node.matches(sel)) node.classList.add('glass', 'interactive');
                                else if (node.querySelectorAll) node.querySelectorAll(sel).forEach(el => el.classList.add('glass', 'interactive'));
                            });
                        } catch (e) {}
                    });
                });
